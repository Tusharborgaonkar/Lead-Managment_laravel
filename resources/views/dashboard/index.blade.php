    ...
    <div class="flex items-center gap-2.5">
        ...
        <div class="relative">
            <button onclick="toggleDropdown('dateFilterDropdown')" id="btnDateFilter"
                ...
                <span id="selectedPeriod">{{ request('period', 'Last 30 days') }}</span>
        ...
        <div class="relative">
            ...
            <div id="exportDropdown" class="hidden absolute top-12 right-0 w-48 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700/60 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                <div class="p-1.5 space-y-0.5">
                    <button onclick="exportReport('CSV', 'Dashboard Report', '{{ route('export.csv') }}')" class="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-colors flex items-center gap-2.5">
                        <i data-lucide="file-text" class="w-3.5 h-3.5"></i> Export as CSV
                    </button>
                    <button onclick="exportReport('Excel', 'Dashboard Report', '{{ route('export.excel') }}')" class="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 rounded-xl transition-colors flex items-center gap-2.5">
                        <i data-lucide="file-spreadsheet" class="w-3.5 h-3.5"></i> Export as Excel
                    </button>
                    <button onclick="exportReport('PDF', 'Dashboard Report', '{{ route('export.pdf') }}')" class="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 hover:text-rose-600 dark:hover:text-rose-400 rounded-xl transition-colors flex items-center gap-2.5">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-sm p-6">
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="relative">
                <button onclick="toggleDropdown('revenueDateDropdown')" id="btnRevenueDateFilter" class="flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 rounded-lg text-xs font-bold text-slate-500 dark:text-slate-300 border border-slate-100 dark:border-slate-700/50 hover:bg-slate-100 transition-colors">
                    {{ request('period', 'Last 30 days') }} <i data-lucide="chevron-down" class="w-3 h-3"></i>
                </button>
                <div id="revenueDateDropdown" class="hidden absolute top-8 right-0 w-36 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700/60 z-50 overflow-hidden animate-in fade-in duration-200">
                    <div class="p-1 space-y-0.5 mt-1">
                        @foreach(['Today', 'Yesterday', 'Last 7 days', 'Last 30 days', 'This Year'] as $periodLabel)
                        <a href="{{ route('dashboard', ['period' => $periodLabel]) }}" 
                           class="block w-full text-left px-3 py-2 text-[11px] font-bold text-slate-600 dark:text-slate-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-600 rounded-lg transition-colors">
                            {{ $periodLabel }}
                        </a>
                        @endforeach
                    </div>
...
function setPeriod(period) {
    ...
    // Redirect to apply the date filter on the server side
    window.location.href = "{{ route('dashboard') }}?period=" + encodeURIComponent(period);
}
