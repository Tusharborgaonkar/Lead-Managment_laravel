...
<aside id="sidebar" class="sidebar flex flex-col min-h-screen bg-white dark:bg-slate-900 border-r border-slate-200/60 dark:border-slate-700/60 transition-all duration-300 shadow-sm">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-6">
        ...
        <div>
            ...
            <div class="space-y-0.5">
                ...
                <a href="{{ route('deals.index') }}"
                   class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 {{ request()->routeIs('deals.*') ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 font-medium' }}">
                    <i data-lucide="handshake" class="w-4 h-4 flex-shrink-0"></i> Deals
                </a>
        ...
        {{-- Admin --}}
        <div>
            <div class="text-[9px] font-black uppercase tracking-widest text-slate-400 px-3 mb-2">Admin</div>
            <div class="space-y-0.5">
                <a href="{{ route('users.index') }}"
                   class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 {{ request()->routeIs('users.index') || request()->routeIs('users.store') ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 font-medium' }}">
                    <i data-lucide="user-cog" class="w-4 h-4 flex-shrink-0"></i> Users
                </a>

                <a href="{{ route('users.roles') }}"
                   class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 {{ request()->routeIs('users.roles*') ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 font-medium' }}">
                    <i data-lucide="shield" class="w-4 h-4 flex-shrink-0"></i> Roles &amp; Permissions
                </a>

                <a href="{{ route('notifications.index') }}"
                   class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 {{ request()->routeIs('notifications.*') ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 font-medium' }}">
                    <i data-lucide="bell" class="w-4 h-4 flex-shrink-0"></i> Notifications
                    <span class="ml-auto text-[9px] font-black bg-indigo-500 text-white px-1.5 py-0.5 rounded-full">3</span>
                </a>

                <a href="{{ route('settings.index') }}"
                   class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 {{ request()->routeIs('settings.*') ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/60 font-medium' }}">
                    <i data-lucide="settings" class="w-4 h-4 flex-shrink-0"></i> Settings
                </a>
            </div>
        </div>
